...
fn self_spend_impl(test_dir: &'static str) -> Result<(), wallet::Error> {
	...
	wallet::controller::owner_single_use(Some(wallet1.clone()), mask1, None, |api, m| {
		...
		assert_eq!(wallet1_info.total, 4 * award);
		// assert_eq!(
	...
	wallet::controller::owner_single_use(Some(wallet1.clone()), mask1, None, |api, m| {
		...
		assert_eq!(wallet1_info.total, 1); //todo why the amount increase totally?
		Ok(())
	})?;
	...
	println!(
		"===========afterwards the outputs length are{}",
		output_list_after_spend.len()
	);
	assert!(output_list_after_spend.len() == 4);
